...
export const validateLegacyLeadForm = (data: Partial<LeadFormInput>): { isValid: boolean; errors: Record<string, string> } => {
  const errors: Record<string, string> = {};
  const name = (data.name || '').trim();
  const phone = (data.phone || '').trim();
  const country_code = data.country_code || DEFAULT_COUNTRY_CODE.code;
  const home_type = (data.home_type || '').trim();
  ...
  } else if (phone.length !== reqDigits) {
    errors.phone = 'Must be ' + reqDigits + ' digits for ' + country.name;
  }
  ...
  if (notes.length > 300) {
    errors.notes = 'Notes must be under 300 characters';
  }
